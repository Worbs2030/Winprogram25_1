...
bool checkCollision(Object mosnters[], int i); //충돌 감지 함수

// 캐릭터 히트박스 디버깅 함수
void printHitbox(Object player)
{
    printf("플레이어 히트박스: x=%d, y=%d, width=%d, height=%d, HP=%d, attack=%d, attackPower=%d\n",
        player.hitbox[1].x, player.hitbox[1].y, player.hitbox[1].width, player.hitbox[1].height, player.health, player.attackState, player.attackPower);
}
...
void initializeMap(char map[MAP_HEIGHT][MAP_WIDTH], Object player, Object monsters[], int cameraOffset)
{
    ...
    for (i = 0; i < MONSTER_COUNT; i++)
    {
        if (checkCollision(&monsters[i], i))
        {
            char str[10]; // 종료 전 입력
            printf("충돌 발생! 게임 종료.\n");
            // 사용자 입력을 받기 위한 코드 추가
...
bool checkCollision(Object monsters[], int i)
{

    Hitbox hitboxA0 = player.hitbox[0];
    Hitbox hitboxA1 = player.hitbox[1];
    Hitbox b = monsters[i].hitbox[0];

    // a[0]이 유효한 경우
    if (hitboxA0.width > 0 && hitboxA0.height > 0)
    {
        ...
        {
            monsters[i].health -= player.attackPower; // HP 감소
            return true; // 충돌 발생
        }
    }

    // a[1]이 유효한 경우
    if (hitboxA1.width > 0 && hitboxA1.height > 0)
    {
